---
name: audit-agents
description: Audit and maintain agent configuration files (.Codex/agents, AGENTS.md, AGENTS.md, agent-memory). Checks for duplicates, outdated info, naming conflicts, wrong technical facts, and files over 200 lines. Fixes all issues and commits.
---
...
- `.Codex/agents/*.md` — one per agent role

**Agent memory files:**
- `.Codex/agent-memory/*/MEMORY.md` — persistent cross-session memory per agent
...
- Any other `AGENTS.md` or `AGENTS.md` files in the repo (`Glob **/AGENTS.md` and `Glob **/AGENTS.md`)
...
### 1. Naming conflicts
- Read frontmatter of every `.Codex/agents/*.md`
...
- The persistent memory instructions (Guidelines, What to save, What NOT to save, Explicit user requests) are already part of Codex's built-in behavior when `memory: project` is set. Do NOT duplicate this ~35-line block in every agent file. Remove it from agent files.
...
### 5. Information already in Codex's built-in system prompt
The following is provided by Codex itself and should NOT be repeated in agent files:
...
- Check that each directory under `.Codex/agent-memory/` has a corresponding `.Codex/agents/*.md` file.
- If memory exists in an unexpected location (e.g., `backend/.Codex/agent-memory/`), determine whether it's the correct location or a duplicate. The canonical location is `{project-root}/.Codex/agent-memory/{agent-name}/MEMORY.md`.
